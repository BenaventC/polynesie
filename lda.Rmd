...
Dans cette note de recherche nous analysons les commentaires déposés sur Tripadvisor et relatifs à un hôtel en particulier (On fera tourner plus tard la procédure sur l'intégralité des commentaires). 

L'objectif est d'abord de décrire le contenu de ces textes, puis d'en analyser la tonalité avec plusieurs outils distincts et ainsi d'en évaluer la consistance. On s'intéressera notamment à la relation entre ces indicateurs et la note données par les évaluateurs.
...
Le texte à l'état brut comporte de nombreux signes qui amènent peu de sens lexical : des points , des noms de lieux, des déterminants. La stratégie que nous engageons est différentes de cette de tm qui juxtapose : mise en minuscule, suppression des ponctuactions, des mentions et des ref html, notre stratégie n'élimine pas mais se concentre sur les éléments lexicaux les plus signifiants. (au moins de manières premières, les autres éléments secondaires peuvent révéler des inflexions plus fines, comme l'usage de la ponctuation le permet - Proust l'aura démontré ref  les coutures proust)
...
L'expérience reste largement inexprimée si ce n'est pas un jugement très positif (bien, bon, beau) qui nous incite à explorer dans une prochaine section les sentiments et émotions associées. Mais auparavant tentons d'isoler dans ces ensembles de termes des sujets d'évaluation.
...
![Topic](LDAvis01.jpg){ width=75% }


 * En cliquant sur les mots on peut aussi observer leur distribution dans les différents groupes et peut en apprécier la spécificité.
 
![Topic](LDAvis02.jpg){ width=75% }
...
On s'intéressera à des dépendences de qualification en optant pour la dépendence "amod" :  adjectival modifier - An adjectival modifier of an NP is any adjectival phrase that serves to modify the meaning of the NP.“Sam eats red meat”amod(meat, red) et "admod" : adverbial modifier. 

Pour plus de détail il faut consulter le Stanford typed dependencies manual Marie-Catherine de Marneffe and Christopher D. Manning September 2008 Revised for Stanford Parser v.  1.6.2 in February 2010  on en trouvera un exposé ici [universal dependencies](http://universaldependencies.org/u/dep/).
...
On représente les qualificatifs de 4 notions qui apparaissent comme étant les plus fréquentes au travers de leur manifestation par l'usage de noms communs : le logement, son environnement, l'hôte et le séjour.
...
gd2<- ggplot() + 
  ...
  theme(legend.position="none")+ labs(title="Qualificatifs des lemmes 'hotel' et 'chambre'")+coord_equal()
...
#place
dep<-cnlp_get_dependency(obj, get_token = TRUE)
res <- dep %>%filter(relation == "amod")%>%filter(lemma == "plage")
bub<-res$lemma_target %>%table() %>%sort(decreasing = TRUE) %>%head(n = 30)%>% as.data.frame()
# libraries library(packcircles)
# Generate the layout
packing <- circleProgressiveLayout(bub$Freq, sizetype='area')
...
gd4<- ggplot() + 
  ...
  theme(legend.position="none")+ labs(title="Qualificatifs du lemme 'plage' ")+coord_equal()
...
ggplot(df_en, aes(x=sentiment))+geom_histogram(binwidth=1,aes(fill=quintile))+theme_minimal()+xlim(-15,+30) +scale_fill_manual(values=Zissou)
```
...
# 10. modèle naives bayes pour prédire la note

On utilise un modèle très simple. Le plus basic qu'il soit. 

#https://fderyckel.github.io/2016-12-07-Texts_Classification_in_R/


## 10.1 Constitution du corpus
...
## 10.3 selectioner les termes les plus fréquents to build the DTM
...
ggplot(postprob, aes(x=V1,y=delta))+geom_point(col="grey")+ geom_text(aes(label=ifelse(abs(delta)>0.05,as.character(Name),'')),size =2.5,hjust=0,vjust=0)+scale_x_log10()+theme_minimal()+labs(x="Probabilité a posteriori que le termes soit associé à une bonne note")
```
